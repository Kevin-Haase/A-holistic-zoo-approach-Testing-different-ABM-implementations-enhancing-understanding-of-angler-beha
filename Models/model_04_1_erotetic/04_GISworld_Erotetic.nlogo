...
angler-own [
  ...
  mean_angler_catch       ; mean catch of a angler
]
...
to go-fishing
  ask angler [
    ...
    if ((random-float 1) < (9 / 365))[                                         ; set the probability to go on a fishng trip that on average 9 trips are taken per year
      ...
      set mean_angler_catch (total_angler_catch / total_angler_trips)
...
to choose-location
  ...
  ; go somewhere where my fishing method is possible
  ...
  ; go somewhere with an acceptable distance
  let max_dist max_distance                                                            ; safe the max distance of the current angler as variable
  let acceptable_dist_locations possible_locations with [report-dist <= max_dist]      ; select all locations from the possible locations that are within the max distance

  ifelse (count acceptable_dist_locations > 0)
  ;with acceptable dist locations
  [ let mean_catch mean_angler_catch
    let acceptable_catch_locations acceptable_dist_locations with [CPUE_mean >= mean_catch]      ; pick only locations with mean expected catch above mean catch of angler

    ifelse (count acceptable_catch_locations > 0)
    [ print "Location chosen with acceptable distance and catch"                                 ; if one or more have higher expected mean catch
      set chosen_location one-of acceptable_catch_locations]
    [ print "Location chosen with acceptable distance only"                                      ; if none have higher expected mean catch
      set chosen_location one-of acceptable_dist_locations]
  ]
  ; without acceptable dist locations
  [ let mean_catch mean_angler_catch
    let acceptable_catch_locations possible_locations with [CPUE_mean >= mean_catch]             ; pick only locations with mean expected catch above mean catch of angler

    ifelse (count acceptable_catch_locations > 0)
    [ print "Location chosen with acceptable catch only"                                         ; if one or more have higher expected mean catch
      set chosen_location one-of acceptable_catch_locations]
    [ print "Location chosen with same fishing method only"                                      ; if none have higher expected mean catch
      set chosen_location one-of possible_locations]
  ]
...
; reporter to report the distance between a angler homelocation and a fishing location
to-report report-dist
  set current_location who
  let dist [distances] of connection current_location current_angler
  report dist
end

...
```{r}
setwd("C:/Users/haase_k/Desktop/SVNKevin/PhD-Anglerverhalten/modelling/Angler Model/NetLogo/model_04_1_Erotetic")
```
...
```{r}
Erotetic_angler <- read_csv("model_output/angler.csv")
Erotetic_angler <- separate(data = Erotetic_angler, col = run_ID, into = c("run", "rep"), sep = "_", remove = F)

Erotetic_location <- read_csv("model_output/location.csv")
Erotetic_location <- separate(data = Erotetic_location, col = run_ID, into = c("run", "rep"), sep = "_", remove = F)

Erotetic_parameter <- Erotetic_location %>%
  group_by(run) %>%
  summarise()
```
...
test <- filter(.data = Erotetic_location, visited == TRUE)
test <- test %>%
...
Erotetic_parameter <- left_join(Erotetic_parameter, test)
```
...
```{r}
#for each experiment
test <- Erotetic_location %>%
  group_by(run_ID, location_method)%>%
  mutate(n.total.fishingday.exp = sum(total_visits))

test <- mutate(.data = test, perc_exp = total_visits/n.total.fishingday.exp)
...
```{r}
Erotetic_location <- mutate(.data = Erotetic_location, ang.per.day = total_visits/365)
...
```{r}
names(Erotetic_parameter)[names(Erotetic_parameter) == "location_method"] <- "fishing_method"
Erotetic_parameter <- left_join(Erotetic_parameter, test2)

write.table(Erotetic_parameter, "model_output/Erotetic_results.csv", sep = ",", row.names = FALSE)
```
...
```{r}
test <- left_join(Erotetic_parameter, real_values)
...
test <- subset(x = test, select = c(run, fishing_method, dif.n.location, dif.mean, dif.med, dif.sd, dif.min, dif.max, mean.diff))
```
...
perc.diff <- test %>%
  group_by(run, fishing_method, perc.diff.n.locations, perc.diff.mean, perc.diff.med, perc.diff.sd, perc.diff.min, perc.diff.max, sum.perc.diff) %>%
  summarise()
```
...
perc.diff <- subset(x = perc.diff, select = c(run, fishing_method, perc.diff.n.locations, perc.diff.mean, perc.diff.med, perc.diff.sd, perc.diff.min, perc.diff.max, mean.perc.diff.dist, sum.perc.diff))
```

```{r}
write.table(perc.diff, "model_output/Erotetic_perc_diff.csv", sep = ",", row.names = FALSE)
```

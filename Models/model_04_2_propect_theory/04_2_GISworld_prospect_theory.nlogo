...
globals [
  ...
  parameter_PTa                                 ; parameter alpha from the prospect theory that determines the subjective value for gains
  parameter_PTb                                 ; parameter beta from the prospect theory that determines the subjective value for losses
  parameter_PTl                                 ; parameter lamda from the prospect theory that determines the subjective value for losses
  parameter_up_pos                              ; paramter upsilon for gains from the weighting function of the prospect theory that determines the decision weight
  parameter_up_neg                              ; paramter upsilon for loss from the weighting function of the prospect theory that determines the decision weight
...
angler-own [
  ...
  total_utility      ; total utility received from all fishing trips
  mean_utility       ; mean_utility received from all trips
...
to setup
  ...
  set parameter_PTa value-of-parameter_PTa          ; parameter that determines the increase of the subjective value for gains
  set parameter_PTb value-of-parameter_PTb          ; parameter that determines the decrease of the subjective value for losses
  set parameter_PTl value-of-parameter_PTl          ; parameter that determines the steper decrease of the subjective value for losses compared to gains
  set parameter_up_pos value-of-parameter_up_pos    ; parameter that determines the decision weight for gains
  set parameter_up_neg value-of-parameter_up_neg    ; parameter that determines the decision weight for losses
  ...
  set spot_dataset gis:load-dataset "gis_data/accesspoints/accesspoints_pt.shp"            ; important to use the accesspoint_pt shapefile, which include the catch sd and the probability of an catch over the mean.
...
to go-fishing
  ask angler [
    ...
    if ((random-float 1) < (9 / 365))[                                         ; set the probability to go on a fishng trip that on average 9 trips are taken per year
      choose-location                                                          ; procedure to select a specific location (see below)
      ...
      get-utility                                                              ; procedure to caluclate the received utility from the current fishing trip
...
to choose-location
  ...
  let min_loc_val min ([location_value] of possible_locations)                                                  ; estimate the smallest location value
  set chosen_location rnd:weighted-one-of possible_locations [location_value - min_loc_val]                     ; angler chose the location of the possible locations (same fishing method) with a propapility proportinoal to the location value. - the smallest value is needed as the location values can´t be negativ, therefore the smallest value is set to 0.
...
to-report location_value
  report subjective_location_utility * decision_weight
end



; reporter to caluclate the subjective utility of each possible location
to-report subjective_location_utility
  ifelse (location_utility >= [mean_utility] of a-angler current_angler)                 ; defines if a location utility is a gain or loss compared to the mean utility of the angler
  [let subjective_utility (utility_change ^ parameter_PTa)                               ; if the utility is a gain, the subjective utility is calculated with the gain function and reported
  report subjective_utility]
  [let subjective_utility ((- parameter_PTl) * (- utility_change) ^ parameter_PTb)       ; if the utility is a loss, the subjective utility is calculated with the loss function and reported
  report subjective_utility]
end

; reporter to caluculate the utility difference from the location utility to the current mean utility of an angler
to-report utility_change
  report location_utility - [mean_utility] of a-angler current_angler             ; the difference transform the utility value of an location to gains (positive values) or losses (negative values)
end



to-report decision_weight
  let prob prob_over_mean

  ifelse (location_utility >= [mean_utility] of a-angler current_angler)
  [let decision-weight ((prob ^ parameter_up_pos) / (prob ^ parameter_up_pos + (1 - prob) ^ parameter_up_pos) ^ (1 / parameter_up_pos))
  report decision-weight]
  [let decision-weight ((prob ^ parameter_up_neg) / (prob ^ parameter_up_neg + (1 - prob) ^ parameter_up_neg) ^ (1 / parameter_up_neg))
  report decision-weight]
...
; procedure to calculate the mean utility of an angler, this mean utility is used to determine if a utility of an location in a future site choice is classified as loss or gain
to get-utility
  let received_catch_utility daily_angler_catch * catch_utility_b

  let c_loc [who] of chosen_location
  let dist [distances] of connection c_loc current_angler
  let received_dist_utility 1 - dist * dist_utility_b

  let new_utility parameter_a * received_catch_utility + (1 - parameter_a) * received_dist_utility
  set total_utility (new_utility + total_utility)

  set mean_utility total_utility / total_angler_trips
end
@#$#@#$#@
GRAPHICS-WINDOW
199

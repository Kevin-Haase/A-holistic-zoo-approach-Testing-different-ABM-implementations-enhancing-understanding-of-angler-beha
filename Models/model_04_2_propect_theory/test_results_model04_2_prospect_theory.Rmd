---
title: "test_results_model04_prospect_theory"
author: "Kevin Haase"
date: "3/17/2023"
...
```{r}
setwd("C:/Users/haase_k/Desktop/SVNKevin/PhD-Anglerverhalten/modelling/Angler Model/NetLogo/model_04_2_prospect_theory")
```
...
```{r}
prospect_theory_angler <- read_csv("model_output/angler.csv")
prospect_theory_angler <- separate(data = prospect_theory_angler, col = run_ID, into = c("run", "rep"), sep = "_", remove = F)

prospect_theory_location <- read_csv("model_output/location.csv")
prospect_theory_location <- separate(data = prospect_theory_location, col = run_ID, into = c("run", "rep"), sep = "_", remove = F)

prospect_theory_parameter <- prospect_theory_location %>%
  group_by(run, `value-of-parameter_a`) %>%
  summarise()
```
...
test <- filter(.data = prospect_theory_location, visited == TRUE)
test <- test %>%
...
prospect_theory_parameter <- left_join(prospect_theory_parameter, test)
```
...
```{r}
prospect_theory_location <- mutate(.data = prospect_theory_location, ang.per.day = total_visits/365)
...
```{r}
names(prospect_theory_parameter)[names(prospect_theory_parameter) == "location_method"] <- "fishing_method"
prospect_theory_parameter <- left_join(prospect_theory_parameter, test2)

write.table(prospect_theory_parameter, "model_output/prospect_theory_04_2_results.csv", sep = ",", row.names = FALSE)
```
...
```{r}
test <- left_join(prospect_theory_parameter, real_values)
...
test <- subset(x = test, select = c(run, `value-of-parameter_a`, fishing_method, dif.n.location, dif.mean, dif.med, dif.sd, dif.min, dif.max, mean.diff))
```
...
perc.diff <- test %>%
  group_by(run, `value-of-parameter_a`, fishing_method, perc.diff.n.locations, perc.diff.mean, perc.diff.med, perc.diff.sd, perc.diff.min, perc.diff.max, sum.perc.diff) %>%
  summarise()
```
...
perc.diff <- subset(x = perc.diff, select = c(run, `value-of-parameter_a`, fishing_method, perc.diff.n.locations, perc.diff.mean, perc.diff.med, perc.diff.sd, perc.diff.min, perc.diff.max, mean.perc.diff.dist, sum.perc.diff))
```
...
```{r}
write.table(perc.diff, "model_output/prospect_theory_04_2_perc_diff.csv", sep = ",", row.names = FALSE)
```

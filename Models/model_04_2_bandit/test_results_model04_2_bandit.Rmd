---
title: "test_results_model04_bandit"
author: "Kevin Haase"
date: "3/17/2023"
...
```{r}
setwd("C:/Users/haase_k/Desktop/SVNKevin/PhD-Anglerverhalten/modelling/Angler Model/NetLogo/model_04_2_bandit")
```
...
```{r}
bandit_angler <- read_csv("model_output/1000runs/angler.csv")
bandit_angler <- separate(data = bandit_angler, col = run_ID, into = c("run", "rep"), sep = "_", remove = F)

bandit_location <- read_csv("model_output/1000runs/location.csv")
bandit_location <- separate(data = bandit_location, col = run_ID, into = c("run", "rep"), sep = "_", remove = F)

bandit_parameter <- bandit_location %>%
...
## Number of fishing days per fishing method

```{r}
test <- bandit_location %>%
  group_by(run_ID, run, location_method)%>%
  summarise(n.fishingsdays = sum(total_visits, na.rm = T))

test %>%
  group_by(run, location_method)%>%
  summarise(n.fishingsdays = mean(n.fishingsdays))
```
...
test <- filter(.data = bandit_location, visited == TRUE)
test <- test %>%
...
bandit_parameter <- left_join(bandit_parameter, test)
```
...
```{r}
bandit_location <- mutate(.data = bandit_location, ang.per.day = total_visits/365)
...
```{r}
names(bandit_parameter)[names(bandit_parameter) == "location_method"] <- "fishing_method"
bandit_parameter <- left_join(bandit_parameter, test2)

write.table(bandit_parameter, "model_output/1000runs/bandit_04_2_results.csv", sep = ",", row.names = FALSE)
```
...
```{r}
test <- left_join(bandit_parameter, real_values)
...
```{r}
write.table(perc.diff, "model_output/1000runs/bandit_04_2_perc_diff.csv", sep = ",", row.names = FALSE)
```

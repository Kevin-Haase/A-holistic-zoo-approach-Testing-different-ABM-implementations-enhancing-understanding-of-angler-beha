...
globals [
  ...
  maximun_distance                              ; parameter to store the maximum distance from a home location to a fishing location
...
to setup
  ...
  set maximun_distance 511                          ; parameter to store the maximum distance from a home location to a fishing location, which is 511km.
...
to go-fishing
  ask angler [
    ...
    set test_a parameter_a
    set test_e parameter_e

    if ((random-float 1) < (9 / 365))[                                         ; set the probability to go on a fishng trip that on average 9 trips are taken per year

      ifelse ((random-float 1) < parameter_e)                                  ; set the probability to explore a random location
      [ explore-location ]                                                     ; procedure to select a random location/explore (see below)
...
to explore-location
  ...
  set chosen_location rnd:weighted-one-of possible_locations [report-dist]                                      ; angler chose the location of the possible locations (same fishing method) with the probability of each location being picked is proportional to the distance
...
to-report report-dist
  ...
  let reverse_dist (maximun_distance - dist)
  report reverse_dist
end

...
locations-own [
  ...
  ; generated during the simulation
  total_visits ; total number of fishing trips to this location
  total_location_catch ; total catch of the whole simulation at this location
]


; a special link between fishing locations and angler homelocation that contains the distance
; a other link could be a social network (not implemented yet)
...
to setup
  ...
  set catch_utility_b 1 / 9                         ; parameter that determines the increase of the catch utility, 9 corresponde to the maximum expected catch of a location which results in the highest utility
  set dist_utility_b 1 / 511                        ; parameter that determines the decrease of the distance utility, 511 corresponde to the maximum distance from a homelocation to a fishing location which result in the lowest utility
  ...
  set spot_dataset gis:load-dataset "gis_data/accesspoints/accesspoints.shp"
...
to setup-locations
  ...
  [["ap_code" "location_name"]["fishing_me" "location_method"]["average_ca" "CPUE_mean"]]
  [
    set color black
    set size 0.2
    set shape "square"
...
to go-fishing
  ask angler [
    ...
    if ((random-float 1) < (9 / 365))[                                         ; set the probility to go on a fishng trip that on average 9 trips are taken per year
      choose-location                                                          ; procedure see below
      ...
      safe-results                                                             ; procedure see below
    ]
  ]
end
...
to choose-location
  ...
  ;set chosen_location max-one-of possible_locations [location_utility]          ; angler chose the location of the possible locations (same fishing method) that has the maximum utility
  set chosen_location rnd:weighted-one-of possible_locations [location_utility]  ; angler chose the location of the possible locations (same fishing method) with the probability of each location being picked is proportional to the utility
...
; procedure to  safe the model outputs, activated in go-fishing
to safe-results
  let model_run behaviorspace-run-number                                                                                  ; safe the behavior space run number to identifie the runs
  let value_a parameter_a                                                                                                 ; safe the value of parameter_a
  let time ticks                                                                                                          ; safe the current tick value as a new variables
  let ID who                                                                                                              ; safe the values of the currently active angler as a new variables
  let home_name homelocation_name
  let fi_method fishing_method
  let chos_location [location_name] of chosen_location                                                                    ; safe the current values of the chosen fishing location from the current angler as a new variables
  let loc_method [location_method] of chosen_location
  let day_catch daily_angler_catch
  file-print csv:to-row (list model_run value_a time ID home_name fi_method chos_location loc_method day_catch)           ; combine the variables to a list, and than write this list in a csv row (the file is opened in the go procedure)
end
@#$#@#$#@
GRAPHICS-WINDOW
199

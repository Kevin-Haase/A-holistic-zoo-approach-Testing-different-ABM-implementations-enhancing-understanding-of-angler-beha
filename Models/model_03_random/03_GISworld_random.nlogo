...
globals [
  ...
  coastline_ycor                                ; y coordinate where the ocean starts
  ...
  ; for behavior space report
  ;ID home_name fi_method chos_location loc_method day_catch
...
to go
  ...
  ;file-open "output_model03.csv"          ; open the file to safe the results
...
to go-fishing
  ask angler [
    ...
    if ((random-float 1) < (9 / 365))[                                         ; set the probility to go on a fishng trip that on average 9 trips are taken per year
      ...
      ifelse safe-result? [ safe-results ] [ print "results not safed" ]       ; procedure see below
    ]
  ]
end
...
to choose-location
  ...
  set chosen_location one-of locations with [location_method = method]          ; chose a random location with the same possible fishing method at the location as the angler
...
to safe-results
  let model_run behaviorspace-run-number
  ...
  file-print csv:to-row (list model_run time ID home_name fi_method chos_location loc_method day_catch) ; combine the variables to a list, and than write this list in a csv row (the file is opened in the go procedure)
end

...
```{r}
setwd("C:/Users/haase_k/Desktop/SVNKevin/PhD-Anglerverhalten/modelling/Angler Model/NetLogo/model_03")
```
...
for (i in 1:100) {
  save_files[[i]] <- read_csv(file = names[i], 
                              col_names = c("model_run", "time", "ID", "homelocation", "fishing_method", "location", "location_method", "catch"), show_col_types = FALSE)
  #print(i)
}
#wendet eine Funktion "rbind" auf eine liste an
netlogo_random <- do.call(rbind, save_files)
netlogo_random$model_run <- as.character(netlogo_random$model_run)

netlogo_random_safe <- netlogo_random
```

## ML3

ML3 model output of 100 runs is importet below.
...
save_files <- vector(mode = "list", length = 100)
#Objekt erzeugen wo die Namen drin stehen wie sie gespeichert herißen
names <- paste("results-M_R-100experiments/config-0/run-", 0:99, ".csv", sep = "")
names
#Alternative
#test <- list.files(file.path("C:/Users/haase_k/Desktop/SVNKevin/PhD-Anglerverhalten/modelling/Angler Model/NetLogo/model_03/model_output"))
...
model_run <- paste("run-", 0:99, sep = "")
save_files <- mapply(cbind, save_files, "model_run"=model_run, SIMPLIFY=F)
#wendet eine Funktion "rbind" auf eine liste an
ml3_random <- do.call(rbind, save_files)

names(ml3_random)[names(ml3_random) == "Angler_fishing_method"] <- "fishing_method"
names(ml3_random)[names(ml3_random) == "Angler_postcode"] <- "homelocation"
names(ml3_random)[names(ml3_random) == "FishingLocation"] <- "location"
names(ml3_random)[names(ml3_random) == "FishingLocation_method"] <- "location_method"

ml3_random_safe <- ml3_random
```
...
```{r}
netlogo_random$model <- "NetLogo"
ml3_random$model <- "ML3"

random <- bind_rows(netlogo_random, ml3_random)
...
```{r}
netlogo_random$location[netlogo_random$location_method != netlogo_random$fishing_method]
ml3_random$location[ml3_random$location_method != ml3_random$fishing_method]
```
...
```{r}
test <- random %>%
  group_by(model, model_run, fishing_method) %>%
...
In ML3 und Netlogo werden alle location angefahren!
...
random_parameter <- left_join(random_parameter, test)
```
...
test <- test %>%
  group_by(model, model_run, fishing_method, n.total.fishingday.exp, location)%>%
...
run <- run %>%
  group_by(model, model_run, location, location_method) %>%
...
test <- run %>%
  group_by(model, model_run, location_method)%>%
...
test <- test %>%
  group_by(model, location_method) %>%
...
ml3/netlogo and real world data is needed for this comparison
...
#Netlogo
run <- run %>%
  group_by(model, model_run)%>%
  mutate(n.total.fishingday.exp = length(ID))

run <- run %>%
  group_by(model, model_run, n.total.fishingday.exp, location)%>%
  summarise(n.fishingday.exp = length(ID))
...
test <- mutate(.data = test, diff = test$perc_data - test$perc_exp)
...
test %>%
  ...
  summarise(mean.diff = mean(abs(diff), na.rm = T),
            mean.perc.diff = mean(abs(perc.diff), na.rm = T))
```
...
```{r}
run <- run %>%
  group_by(model, fishing_method, zip_code) %>%
...
```{r}
random_parameter <- left_join(random_parameter, test2)

write.table(random_parameter, "model_output/random_results.csv", sep = ",", row.names = FALSE)
```
...
```{r}
test <- left_join(random_parameter, real_values)
...
test <- subset(x = test, select = c(model, fishing_method, dif.n.location, dif.mean, dif.med, dif.sd, dif.min, dif.max, mean))
```
...
perc.diff <- test %>%
  group_by(model, fishing_method, perc.diff.n.locations, perc.diff.mean, perc.diff.med, perc.diff.sd, perc.diff.min, perc.diff.max, sum.perc.diff) %>%
  summarise()
```
...
perc.diff <- subset(x = perc.diff, select = c(model, fishing_method, perc.diff.n.locations, perc.diff.mean, perc.diff.med, perc.diff.sd, perc.diff.min, perc.diff.max, mean.perc.diff.dist, sum.perc.diff))
```



```{r}
write.table(perc.diff, "model_output/random_perc_diff.csv", sep = ",", row.names = FALSE)
```

...
pacman::p_load(readr, dplyr, tidyr, reshape2, ggplot2, lhs, nlrx, future)

```

```{r}
setwd("~/NetLogo/model_04_bandit")
```
...
netlogopath <- file.path("/home/khaase/Desktop/NetLogo 6.3.0")
modelpath <- file.path("/home/khaase/NetLogo/model_04_bandit/04_GISworld_bandit.nlogo")
outpath <- file.path("/home/khaase/NetLogo/model_04_bandit/model_output")
...
plan(multisession)
results_bandit <- run_nl_all(nl = nl, split = 7)
...
#setsim(nl, "simoutput") <- results_bandit
#write_simoutput(nl)
```
...
write.table(location, "model_output/location.csv", sep = ",", row.names = FALSE)
```
...
write.table(angler, "model_output/angler.csv", sep = ",", row.names = FALSE)
```
